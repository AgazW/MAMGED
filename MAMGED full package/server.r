...
validate_range <- function(af_files, affy_annotation, affy_range){
  ...
  if(length(vec) > length(affy_annotation)) 
 ...
 observeEvent(input$Submit,{
  ...
  list_files <- eventReactive(input$Submit,{
    ...
    if(!file.exists(input$files[['datapath']][1]))
      ...
      new_file_names <- input$files[['datapath']][1]
    lines <- readLines(new_file_names[1],10)
    ...
    withProgress(message = 'Processing Data . . .',{
      if(all(CheckforCEL) && !input$checkbox)
      {
        ...
        validate_range(input$files[['datapath']], input$dataset, input$Range)
      ...
      else if(all(CheckforTXT) && !input$checkbox)
      {
        if(input$radio == 1){
      ...
      else if(all(CheckforCEL)|all(CheckforTXT) && input$checkbox){
        stop("No target files found, please check target files and try again")
      }
      else if(all(CheckforIlluminaDE) && input$checkbox){
